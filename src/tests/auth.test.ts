...
test("Logout end point", async () => {
  const mockFindById = jest.spyOn(userModel, "findById").mockResolvedValueOnce(null);

  // Generate a valid refresh token
  const validRefreshToken = jwt.sign({ _id: "nonexistentUserId" }, process.env.TOKEN_SECRET!, {
    expiresIn: "1h",
  });
